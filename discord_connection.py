...
class PollCreateView(discord.ui.View):
    ...
    async def request_ballot(
        ...
        if user.id in self.used_users:
            logger.debug(
                "user has requested ballot more than once", user.id, self.poll_id
            )
            await interaction.response.send_message(
                "You already used this button.", ephemeral=True
            )
            return
        ...
        if candidates is None:
            await interaction.response.send_message(
                "Poll no longer exists!", ephemeral=True
            )
            return
        ...
        try:
            ...
            msg = (
                f"Hello! Please reply with responses for poll #**{self.poll_id}**. "
            ...
            msg += "```"
            embed = discord.Embed(
                title=f"Poll #{self.poll_id}", description=str(msg)[:4096]
            )
            await dm.send(embed=embed)

            message_result = False
            while message_result == False:
                message_result = await await_vote(
                    self.poll_id, user.id, interaction, dm
                )
            ...
            await interaction.followup.send(
                "I can't DM you. Please enable DMs.", ephemeral=True
            )
...
async def await_vote(
    poll_id: int, user_id: int, interaction: discord.Interaction, dm: DMChannel
):
    ...
    reply = await interaction.client.wait_for(
        "message", check=check, timeout=60 * 60 * 24
    )

    res = record_vote(poll_id, user_id, reply.content.strip())
    if res is None:
        await dm.send(
            "Either the poll is closed, or you already voted. Your last vote has not been recorded."
        )
        return None
    elif res is False:
    ...
    embed = discord.Embed(
        title=f"Poll #{poll_id} vote",
        description=str(f"I've recorded your ballot:\n ```{user_ballot}```")[:4096],
    )
    await dm.send(embed=embed)
...
async def poll_create(
    interaction: discord.Interaction, poll_title: str, poll_entries: str
):
    ...
    if candidates is None:
        await interaction.response.send_message(
            "Something went wrong when fetching candidates. Please try creating another poll."
        )
        return
...
async def poll_close(interaction: discord.Interaction, poll_id: int):
    ...
    msg = pretty_print_results(result)
    print(msg)
    embed = discord.Embed(
        title=f"Poll #{poll_id} Results:", description=str(f"```{msg}```")[:4096]
    )
